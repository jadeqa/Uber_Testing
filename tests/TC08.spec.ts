...
// TC08: Lead conversion with existing account selection and merge validation

test('TC08: Lead conversion with choose existing account and merge validation', async ({ page }) => {
  ...
  // 11. Verify successfully lead creation message
  await expect(page.locator('.toast-message')).toContainText('Lead converted successfully');

  // 12. Verify that new account is not created, lead gets merged
  // This step may require checking that the account count did not increase, or that the lead is now associated with the existing account
  // Example assertion (update selector as needed):
  await expect(page.locator('#conversionStatus')).toHaveText('Converted');
  // Optionally, check for absence of new account creation
  // await expect(page.locator('.new-account-indicator')).toHaveCount(0);
});
