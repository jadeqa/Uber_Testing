...
test('Salesforce Lead Creation and Conversion - CPQ User', async ({ page }) => {
  page.setDefaultTimeout(5000);

  // 1. Login with CPQ user
  ...
  // 9. Click on contact link
  await page.click('a:has-text("Contact")');

  // 10. Verify contact details match lead
  const contactName = await page.locator('span[data-field="Contact Name"]').innerText();
  ...
  expect(contactName).toContain('abd');
